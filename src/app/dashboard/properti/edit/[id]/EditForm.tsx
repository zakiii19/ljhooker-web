...
import { updateProperti, deleteProperti } from '../../actions'
import { AlertCircle, ArrowLeft, Save, Trash2, UploadCloud, X, ChevronLeft, ChevronRight, Plus } from 'lucide-react'
...
export default function EditPropertiForm({
  properti,
  marketingAgents
}: {
  properti: any
  ...
  const [isForSale, setIsForSale] = useState(properti.is_for_sale)
  const [isForRent, setIsForRent] = useState(properti.is_for_rent)
  const [isExclusive, setIsExclusive] = useState(properti.is_exclusive)

  // Initialize images with existing ones from DB
  const [images, setImages] = useState<ImageItem[]>([])

  useEffect(() => {
    if (properti.properti_media) {
      const supabase = createClient()
      const initialImages = properti.properti_media.map((m: any) => {
        let previewUrl = m.media_url
        
        // 1. Jika URL menggunakan nama bucket lama (properti-media), sesuaikan ke properti_media
        if (previewUrl && previewUrl.includes('/properti-media/')) {
          previewUrl = previewUrl.replace('/properti-media/', '/properti_media/')
        }
        
        // 2. Jika URL adalah path relatif, ambil public URL dari Supabase
        if (previewUrl && !previewUrl.startsWith('http')) {
          previewUrl = supabase.storage.from('properti_media').getPublicUrl(previewUrl).data.publicUrl
        }
  ...
  // Cleanup object URLs for newly added files when they are removed/unmounted
  useEffect(() => {
    return () => {
      images.forEach((img) => {
        if (!img.isExisting) {
  ...
  const handleDelete = async () => {
    if (confirm('Apakah Anda yakin ingin menghapus properti ini secara permanen?')) {
      setIsDeleting(true)
      try {
        const result = await deleteProperti(properti.id)
        if (result && result.error) {
          setError(result.error)
          setIsDeleting(false)
        }
      } catch (err: any) {
        setError(err.message || 'Gagal menghapus properti.')
        setIsDeleting(false)
      }
    }
  }
      ...
      try {
        ...
        for (let i = 0; i < images.length; i++) {
          ...
          if (item.isExisting) {
            ...
            finalUrls.push(item.preview)
          } else if (item.file) {
            // Compress and upload new image
            setUploadStatus(`Mengompresi gambar baru ke-${i + 1}...`)
            const compressedBlob = await compressImage(item.file)

            setUploadStatus(`Mengunggah gambar baru ke-${i + 1} dari ${images.length}...`)
            
            const filename = `properties/${properti.id}/${i}_${Date.now()}.jpg`
            ...
            if (uploadError) {
              throw new Error(`Gagal mengunggah gambar baru ${i + 1}: ${uploadError.message}`)
            }
        ...
        setUploadStatus('Menyimpan perubahan properti...')

        const result = await updateProperti(properti.id, null, formData)
        if (result && result.error) {
          setError(result.error)
        }
      } catch (err: any) {
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <input
                ...
                type="text"
                required
                defaultValue={properti.no_listing}
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                type="text"
                required
                defaultValue={properti.pemilik_nama}
            ...
            <div className="space-y-1.5">
              ...
              <select
                name="tipe_properti"
                defaultValue={properti.tipe_properti}
            ...
            <div className="space-y-1.5">
              ...
              <select
                name="status_aktif"
                defaultValue={properti.status_aktif}
                className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-zinc-300 outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]"
              >
              ...
              {isForSale && (
                <div className="space-y-1.5 animate-fadeIn">
                  ...
                  <input
                    ...
                    type="number"
                    required
                    defaultValue={properti.harga_jual || ''}
              ...
              {isForRent && (
                <div className="space-y-1.5 animate-fadeIn">
                  ...
                  <input
                    ...
                    type="number"
                    required
                    defaultValue={properti.harga_sewa || ''}
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <select
                ...
                defaultValue={properti.tipe_sertifikat}
                className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-zinc-300 outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]"
              >
        ...
        <div className="space-y-5 border-t border-white/5 pt-6">
          ...
          <div className="grid gap-5 grid-cols-2 sm:grid-cols-4">
            <div className="space-y-1.5">
              ...
              <input name="luas_tanah" type="number" step="any" defaultValue={properti.luas_tanah || ''} placeholder="0" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Luas Bangunan (m²)</label>
              <input name="luas_bangunan" type="number" step="any" defaultValue={properti.luas_bangunan || ''} placeholder="0" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Lebar Tanah (m)</label>
              <input name="lebar_tanah" type="number" step="any" defaultValue={properti.lebar_tanah || ''} placeholder="0" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Panjang Tanah (m)</label>
              <input name="panjang_tanah" type="number" step="any" defaultValue={properti.panjang_tanah || ''} placeholder="0" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
          </div>
          ...
          <div className="grid gap-5 sm:grid-cols-4">
            <div className="space-y-1.5">
              ...
              <input name="jumlah_lantai" type="number" defaultValue={properti.jumlah_lantai || 1} className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Hadap Arah</label>
              <input name="hadap" type="text" defaultValue={properti.hadap || ''} placeholder="Utara/Selatan/dll" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Kondisi Properti</label>
              <input name="kondisi" type="text" defaultValue={properti.kondisi || ''} placeholder="Bagus, Butuh Renovasi, dll" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Kapasitas Listrik (VA)</label>
              <input name="listrik_va" type="number" defaultValue={properti.listrik_va || ''} placeholder="2200" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
          </div>
          ...
          <div className="flex gap-6 pt-2">
            <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-300">
              <input type="checkbox" name="has_pam" defaultChecked={properti.has_pam} className="h-4 w-4 accent-yellow-400 rounded cursor-pointer" />
              <span>Sumber Air PAM</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-300">
              <input type="checkbox" name="has_sumur" defaultChecked={properti.has_sumur} className="h-4 w-4 accent-yellow-400 rounded cursor-pointer" />
        ...
        <div className="space-y-5 border-t border-white/5 pt-6">
          ...
          <div className="grid gap-5 grid-cols-2 sm:grid-cols-4">
            <div className="space-y-1.5">
              ...
              <input name="kamar_tidur_standard" type="number" defaultValue={properti.kamar_tidur_standard || 0} placeholder="0" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">KT Pembantu</label>
              <input name="kamar_tidur_pembantu" type="number" defaultValue={properti.kamar_tidur_pembantu || 0} placeholder="0" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">KM Standard</label>
              <input name="kamar_mandi_standard" type="number" defaultValue={properti.kamar_mandi_standard || 0} placeholder="0" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">KM Pembantu</label>
              <input name="kamar_mandi_pembantu" type="number" defaultValue={properti.kamar_mandi_pembantu || 0} placeholder="0" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
          </div>
          ...
          <div className="grid gap-5 grid-cols-2 sm:grid-cols-4">
            <div className="space-y-1.5">
              ...
              <input name="garasi" type="number" defaultValue={properti.garasi || 0} placeholder="0" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Kapasitas Carport</label>
              <input name="carport" type="number" defaultValue={properti.carport || 0} placeholder="0" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Jumlah AC Split</label>
              <input name="ac_split_count" type="number" defaultValue={properti.ac_split_count || 0} placeholder="0" className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]" />
            </div>
          </div>
          ...
          <div className="grid gap-4 grid-cols-2 sm:grid-cols-4 pt-2">
            <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-300">
              <input type="checkbox" name="dapur_basah" defaultChecked={properti.dapur_basah} className="h-4 w-4 accent-yellow-400 rounded cursor-pointer" />
              <span>Dapur Basah</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-300">
              <input type="checkbox" name="dapur_kering" defaultChecked={properti.dapur_kering} className="h-4 w-4 accent-yellow-400 rounded cursor-pointer" />
              <span>Dapur Kering</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-300">
              <input type="checkbox" name="taman_depan" defaultChecked={properti.taman_depan} className="h-4 w-4 accent-yellow-400 rounded cursor-pointer" />
              <span>Taman Depan</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-300">
              <input type="checkbox" name="taman_belakang" defaultChecked={properti.taman_belakang} className="h-4 w-4 accent-yellow-400 rounded cursor-pointer" />
              <span>Taman Belakang</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-300">
              <input type="checkbox" name="has_gudang" defaultChecked={properti.has_gudang} className="h-4 w-4 accent-yellow-400 rounded cursor-pointer" />
              <span>Gudang</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-300">
              <input type="checkbox" name="is_furnished" defaultChecked={properti.is_furnished} className="h-4 w-4 accent-yellow-400 rounded cursor-pointer" />
        ...
        <div className="space-y-5 border-t border-white/5 pt-6">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
              ...
              {isExclusive && (
                <div className="space-y-1.5 animate-fadeIn">
                  ...
                  <input
                    ...
                    type="date"
                    required
                    defaultValue={properti.tgl_jatuh_tempo || ''}
                    className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-white outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]"
                  />
                </div>
              )}
            ...
            <div className="rounded-2xl border border-white/5 bg-white/[0.005] p-5 space-y-4">
              ...
              <div className="space-y-1.5">
                <select
                  name="marketing_id"
                  defaultValue={properti.marketing_id || ''}
                  className="w-full rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm text-zinc-300 outline-none focus:border-yellow-400/50 focus:bg-white/[0.08]"
                >
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              name="keterangan"
              rows={4}
              defaultValue={properti.keterangan || ''}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-red-500/10 border border-red-500/20 px-5 py-3 text-sm font-semibold text-red-400 transition-all duration-300 hover:bg-red-500/20 hover:text-red-300 disabled:pointer-events-none disabled:opacity-50"
          >
            {isDeleting ? (
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto justify-end">
          <Link
            ...
            className="flex items-center gap-2 rounded-xl bg-white/5 border border-white/10 px-5 py-3 text-sm font-semibold text-zinc-300 transition-all duration-300 hover:bg-white/10 w-full sm:w-auto justify-center"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-yellow-400 px-6 py-3 text-sm font-semibold text-black transition-all duration-300 hover:bg-yellow-500 hover:shadow-lg hover:shadow-yellow-400/20 active:scale-[0.98] disabled:pointer-events-none disabled:opacity-50 w-full sm:w-auto justify-center"
          >
            {isPending ? (
